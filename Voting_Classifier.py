...
svm_params = {'kernel': 'rbf', 'gamma': 'scale', 'C': 10}
logistic_params = {'solver': 'newton-cg', 'max_iter': 1000, 'penalty': 'l2', 'C': 10}
tree_params = {'min_samples_split': 10, 'min_samples_leaf': 1, 'max_depth': 10, 'criterion': 'gini'}

# Crear clasificadores individuales con hiperparámetros personalizados
svm_clf = SVC(**svm_params, probability=True, random_state=42)
logistic_clf = LogisticRegression(**logistic_params, random_state=42)
tree_clf = DecisionTreeClassifier(**tree_params, random_state=42)

# Crear un clasificador de votación con estimadores personalizados
best_model = VotingClassifier(
    estimators=[('svm', svm_clf), ('logistic', logistic_clf), ('tree', tree_clf)],
    voting='hard'  # 'soft' para ponderación de probabilidades, 'hard' para votación simple
)
